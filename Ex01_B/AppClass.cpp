...
#pragma region YOUR CODE GOES HERE
	...
	//Rotate the shield so that it can go in a circle
	quaternion rotationQuat = quaternion(vector3(0, 0, -fTimer * PI/2.5f));
	matrix4 rotationMat4 = glm::mat4_cast(rotationQuat);
	modelMatrix *= rotationMat4;

	//Move it out the radius of the circle
	modelMatrix *= glm::translate(modelMatrix, vector3(0, 3, 0));

	//Fix its rotation to counteract the inital rotation
	rotationQuat = quaternion(vector3(0, 0, 2 * fTimer * PI / 2.5f));
	rotationMat4 = glm::mat4_cast(rotationQuat);
	modelMatrix *= rotationMat4;
